...
let make_symbol ?compilation_unit name =
  ...
  possible hack is to check for [String.ends_with ~suffix:"frametable" name]. An
  intermediate solution would be to find all the cases where names are
  accessible from the runtime and use the legacy names for them via a separate
  version of [make_symbol]. For now we use legacy mangling here. *)
